...
export default function QCQueueClient() {
  ...
  const fetchJobs = async () => {
    try {
      const res = await fetch('/api/v1/qc-jobs');
      ...
      <table style={{ width: '100%', textAlign: 'left', borderCollapse: 'collapse' }}>
        ...
        <tbody>
          {loading && jobs.length === 0 ? (
            <tr>
              <td colSpan={9} style={{ padding: 'var(--spacing-8)', textAlign: 'center', color: 'var(--text-muted)' }}>Loading QC queue...</td>
            </tr>
          ) : jobs.length === 0 ? (
            <tr>
              <td colSpan={9} style={{ padding: 'var(--spacing-8)', textAlign: 'center', color: 'var(--text-muted)' }}>No QC jobs currently in the queue.</td>
            </tr>
          ) : (
            jobs.map(job => (
              <tr key={job.id} style={{ borderBottom: '1px solid var(--border-light)' }}>
                ...
                <td style={{ padding: 'var(--spacing-3) var(--spacing-4)', fontSize: '0.875rem' }}><span style={{ backgroundColor: 'var(--bg-elevated)', padding: '2px 6px', borderRadius: '4px' }}>{job.jobType}</span></td>
                ...
                <td style={{ padding: 'var(--spacing-3) var(--spacing-4)', fontSize: '0.875rem' }}><span style={{ color: job.status === 'FAILED' ? 'var(--status-error)' : job.status === 'RUNNING' ? 'var(--status-warning)' : job.status === 'COMPLETED' ? 'var(--status-success)' : 'var(--text-muted)' }}>● {job.status}</span></td>
                <td style={{ padding: 'var(--spacing-3) var(--spacing-4)', fontSize: '0.875rem' }}><div style={{ width: '100px', height: '6px', backgroundColor: 'var(--bg-base)', borderRadius: '3px', overflow: 'hidden' }}><div style={{ width: `${job.progress}%`, height: '100%', backgroundColor: 'var(--accent-primary)', transition: 'width 0.3s ease' }}></div></div></td>
                ...
                <td style={{ padding: 'var(--spacing-3) var(--spacing-4)', textAlign: 'right' }}><div style={{ display: 'flex', gap: 'var(--spacing-2)', justifyContent: 'flex-end' }}>{job.status === 'FAILED' && <button className="btn btn-secondary" style={{ padding: '4px 8px', fontSize: '0.75rem' }}>Retry</button>}<button className="btn btn-secondary" style={{ padding: '4px 8px', fontSize: '0.75rem' }}>Logs</button></div></td>
              </tr>
            ))
          )}
        </tbody>
